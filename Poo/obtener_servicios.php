...
// }
if (isset($_POST['tipo_vehiculo']) && isset($_POST['especialidad'])) {
    $tipo = mysqli_real_escape_string($db->conexion, $_POST['tipo_vehiculo']);
    $flujo = mysqli_real_escape_string($db->conexion, $_POST['especialidad']);

    // La consulta ahora filtra por el vehículo Y por el tipo de atención
    $sql = "SELECT id_servicio, nombre_servicio, duracion_horas, especialidad 
            FROM servicios 
            WHERE especialidad = '$flujo' 
            AND (categoria_vehiculo = '$tipo' OR categoria_vehiculo = 'MULTIMARCA' OR categoria_vehiculo = 'TODOS') 
            ORDER BY nombre_servicio ASC";
    
    $res = $db->ejecutar($sql);
    
    if ($db->conexion->affected_rows > 0) {
        echo '<option value="" disabled selected>Seleccione '.strtolower($flujo).' para '.$tipo.'...</option>';
        while ($s = $db->recorrer($res)) {
            echo "<option value='".$s['id_servicio']."' 
                          data-duracion='".$s['duracion_horas']."' 
                          data-especialidad='".$s['especialidad']."'>
        ...
        echo '<option value="">Sin servicios de '.$flujo.' para '.$tipo.'</option>';
    }
}
?>