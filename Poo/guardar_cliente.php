...
if ($_POST) {
    // 1. Datos del Cliente - Sinceramente, usamos ?? para evitar Warnings si el campo no viene
    ...
    $anio       = !empty($_POST['anio']) ? intval($_POST['anio']) : 'NULL'; // Convertimos a número o NULL
    $color      = mysqli_real_escape_string($db->conexion, $_POST['color'] ?? '');
    
    // Aquí estaba el error del "VIN":
    ...
    try {
        ...
        // Sinceramente, si no hay errores previos, el header funcionará perfecto
        $res = empty($_POST['id_cliente']) ? 'ok' : 'edit';
        header("Location: ../clientes.php?res=$res");
        exit();
