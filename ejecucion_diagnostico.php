...
$q_llenados = $db->recorrer($db->ejecutar("SELECT COUNT(*) as total FROM inspeccion_resultados WHERE id_orden = '$id_orden_actual'"));
...
$puede_finalizar = $inspeccion_terminada;
...
<div class="container-fluid px-4 py-4">
    ...
    <div class="card-header bg-white py-3 border-0">
        <h5 class="fw-bold mb-0 text-dark"><i class="fa fa-list-check me-2 text-primary"></i>Pasos de Ejecución de Diagnóstico</h5>
        <small class="text-muted">Complete cada tarea y registre la evidencia fotográfica obligatoria.</small>
    </div>
    <div class="card-body p-0">
        <form action="Poo/guardar_progreso_inspeccion.php" method="POST" enctype="multipart/form-data">
            ...
            <div class="list-group list-group-flush">
                <?php
                // CONSULTA: Traemos los pasos específicos del diagnóstico
                $sql_p = "SELECT p.*, r.estado as estado_guardado, r.foto_evidencia 
                          ...
                          ORDER BY p.orden_paso ASC";
                
                $res_p = $db->ejecutar($sql_p);
                while($p = $db->recorrer($res_p)):
                    $esta_bloqueado = !empty($p['foto_evidencia']);
                    $estado = $p['estado_guardado'] ?? 'PENDIENTE';
                ?>
                <div class="list-group-item py-3 px-4 <?php echo $esta_bloqueado ? 'bg-light-subtle' : ''; ?>">
                    <div class="row align-items-center g-3">
                        <div class="col-12 col-md-6">
                            <div class="d-flex align-items-center">
                                <div class="fw-bold text-primary me-3" style="width: 25px;"><?php echo $p['orden_paso']; ?>.</div>
                                <div class="fw-semibold text-dark"><?php echo $p['descripcion_paso']; ?></div>
                            </div>
                        </div>

                        <div class="col-8 col-md-4">
                            <div class="btn-group btn-group-sm w-100 shadow-sm rounded-pill overflow-hidden border" role="group">
                                <input type="radio" class="btn-check" name="estado[<?php echo $p['id_paso']; ?>]" id="p<?php echo $p['id_paso']; ?>_pend" value="PENDIENTE" <?php echo ($estado == 'PENDIENTE') ? 'checked' : ''; ?> <?php echo $esta_bloqueado ? 'disabled' : ''; ?>>
                                <label class="btn btn-outline-secondary border-0" for="p<?php echo $p['id_paso']; ?>_pend">PENDIENTE</label>

                                <input type="radio" class="btn-check" name="estado[<?php echo $p['id_paso']; ?>]" id="p<?php echo $p['id_paso']; ?>_ok" value="OK" <?php echo ($estado == 'OK') ? 'checked' : ''; ?> <?php echo $esta_bloqueado ? 'disabled' : ''; ?>>
                                <label class="btn btn-outline-success border-0 fw-bold" for="p<?php echo $p['id_paso']; ?>_ok">LISTO / OK</label>
                            </div>
                        ...
                        <div class="col-4 col-md-2 text-end">
                            <?php if($esta_bloqueado): ?>
                                <div class="position-relative d-inline-block">
                                    <img src="img/evidencias/<?php echo $p['foto_evidencia']; ?>" class="rounded-3 border border-success shadow-sm" style="width: 45px; height: 45px; object-fit: cover;">
                                    <span class="position-absolute top-0 start-100 translate-middle badge rounded-circle bg-success p-1 border border-white">
                                        <i class="fa fa-check text-white" style="font-size: 8px;"></i>
                                    </span>
                                </div>
                            <?php else: ?>
                                <label class="btn btn-primary rounded-circle shadow-sm btn-camera-handler d-flex align-items-center justify-content-center mx-auto" style="width: 45px; height: 45px; cursor: pointer;">
            ...
            <div class="p-4 bg-light border-top text-center">
                <button type="submit" class="btn btn-primary btn-lg rounded-pill px-5 fw-bold shadow">
                    <i class="fa fa-save me-2"></i>GUARDAR AVANCE TÉCNICO
                </button>
            </div>
        </form>
    </div>
...
<style>
    ...
    .list-group-item:hover { border-left-color: #0d6efd; background-color: #f0f7ff; }
    .btn-camera-handler { transition: all 0.3s; }
    .btn-camera-handler:active { transform: scale(0.9); }
</style>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        ...
                                        $badge_class = [
                                            ...
                                            'ESPERANDO_CONFIRMACION' => 'bg-info text-info',
                                            'APROBADO' => 'bg-success text-success',
                                            'RECHAZADO' => 'bg-danger text-danger'
                                        ];
                                        $status_text = [
                                            'PENDIENTE' => 'POR ENVIAR',
                                            'ESPERANDO_CONFIRMACION' => 'ENVIADO',
                                            'APROBADO' => 'APROBADO',
                                            'RECHAZADO' => 'RECHAZADO'
                                        ];
                                        $current_status = $h['estado_aprobacion'];
                                        ?>
                                        <span class="badge <?php echo $badge_class[$current_status]; ?> bg-opacity-10 border border-<?php echo explode(' ', $badge_class[$current_status])[0]; ?> rounded-pill px-3" style="font-size: 0.7rem;">
            ...
            <div class="card border-0 shadow-sm rounded-4 mb-4">
                <div class="card-header bg-white py-3 border-0 d-flex justify-content-between align-items-center">
                    ...
                    <div class="text-end">
                        ...
                        <span class="badge bg-success bg-opacity-10 text-success border-success border-opacity-25" style="font-size: 10px;">
